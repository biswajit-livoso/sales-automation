...
const CreatePurchases: React.FC<CreatePurchasesProps> = ({
  editData,
  onBack,
}) => {
  ...
  const handleSave = () => {
    ...
    (editData ? updatePurchase(editData._id, fd) : addPurchase(fd))
      .then((response: any) => {
        toast.success(
          editData
            ? response.data.message || `Purchase updated! `
            : response.data.message || `P0urchase added!`
        );
        if (onBack) onBack();
      })
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <Autocomplete
          ...
          onChange={(_, newValue) =>
            setForm({ ...form, supplier: newValue?._id || "" })
          }
        ...
        {form.items.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              />
              {item.units && (
                <Typography variant="body2">unit {item.units[0]}</Typography>
              )}
        ...
        <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
          ...
          <TextField
            ...
            fullWidth
            onChange={(e) =>
              setForm({ ...form, additionalCharge: e.target.value })
            }
          />
          <TextField
            label="Round Off"
        ...
        <Stack direction="row" spacing={2} sx={{ mt: 2 }}>
          <TextField
            ...
            value={form.grTotal}
            type="number"
            onChange={(e) => setForm({ ...form, grTotal: e.target.value })}
          />
          <TextField
            fullWidth
            label="Paid Amount"
